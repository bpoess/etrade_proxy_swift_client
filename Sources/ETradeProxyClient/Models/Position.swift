...
public struct Product: Sendable, Codable {
    ...
    enum CodingKeys: String, CodingKey {
        case symbol, securityType, securitySubType, callPut, expiryYear,
            expiryMonth, expiryDay, strikePrice, expiryType, productId
    }
    ...
    public init(from decoder: Decoder) throws {
        do {
            ...
            securityType = try container.decodeIfPresent(
                String.self,
                forKey: .securityType
            )
            securitySubType = try container.decodeIfPresent(
                String.self,
                forKey: .securitySubType
            )
            callPut = try container.decodeIfPresent(
                String.self,
                forKey: .callPut
            )
            expiryYear = try container.decodeIfPresent(
                Int32.self,
                forKey: .expiryYear
            )
            expiryMonth = try container.decodeIfPresent(
                Int32.self,
                forKey: .expiryMonth
            )
            expiryDay = try container.decodeIfPresent(
                Int32.self,
                forKey: .expiryDay
            )
            strikePrice = try container.decodeIfPresent(
                Decimal.self,
                forKey: .strikePrice
            )
            expiryType = try container.decodeIfPresent(
                String.self,
                forKey: .expiryType
            )
